...
const translations = {
  en: `      "faq9a": "No, there is no limit. You can download as many videos as you want.",
...
`,
  id: `      "faq9a": "Tidak ada batas. Anda dapat mengunduh video sebanyak yang Anda inginkan.",
...
`,
  es: `      "faq9a": "No hay límite. Puedes descargar tantos videos como desees.",
...
`,
  fr: `      "faq9a": "Non, il n'y a pas de limite. Vous pouvez télécharger autant de vidéos que vous le souhaitez.",
...
for (const [lang, extra] of Object.entries(translations)) {
  const matchStr = `"faq9a": ".*?",`;
  const regex = new RegExp(`"faq9a":\\s*".*?",`);
  
  // Need to ensure we only replace the faq9a within the specific lang block
  // A simple hack is since each faq9a is slightly different per language, we can match the exact one.
}
