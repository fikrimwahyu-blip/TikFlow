...
code = re.sub(r'<Lock className="w-8 h-8 sm:w-10 sm:h-10 text-white" />', '<Info className="w-8 h-8 sm:w-10 sm:h-10 text-white" />', code)
code = re.sub(r'<h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-3">\s*Privacy Policy\s*</h1>', '<h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-3">Cookie Policy</h1>', code)
code = code.replace('TikFlow™ - Last updated: August 2026', 'TikFlow™ - Last updated: March 2026')

old_toc = """<nav className="space-y-1">
...
with open('src/components/CookiePolicy.tsx', 'w') as f:
    f.write(code)
